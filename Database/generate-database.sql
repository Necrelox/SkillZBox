...
CREATE TABLE IF NOT EXISTS `SkillZBox`.`USER` (
  ...
  `activityMessage` VARCHAR(120) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `SkillZBox`.`USER_FRIEND` (
  ...
  `createdAt` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `userUuid` BINARY(16) NOT NULL,
  `friendUuid` BINARY(16) NOT NULL,
  `uuid` BINARY(16) NOT NULL DEFAULT (UUID_TO_BIN(UUID(), TRUE)),
  PRIMARY KEY (`uuid`),
  INDEX `fk_USER_FRIEND_1_idx` (`userUuid` ASC) VISIBLE,
  INDEX `fk_USER_FRIEND_2_idx` (`friendUuid` ASC) VISIBLE,
  UNIQUE INDEX `uuid_UNIQUE` (`uuid` ASC) VISIBLE,
