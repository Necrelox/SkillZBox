...
export class AccountQueries {
    ...
    /** Simple Queries */
    public static async getToken(tokenReflect: Partial<User.IToken>): Promise<User.IToken[]> {
    ...
    public static async getUser(userReflect: Partial<User.IUser>): Promise<User.IUser[]> {
    ...
    /** Transaction Queries */
    private static async addUserTransaction(user: Partial<User.IUser>, trx: Transaction) {
    ...
    private static async getUserTransaction(where: Partial<User.IUser>, trx: Transaction): Promise<User.IUser[]> {
        return DatabaseKnex.getInstance().select().from('USER').where(where).transacting(trx);
    }

    private static async getTokenTransaction(where: Partial<User.IToken>, trx: Transaction): Promise<User.IToken[]> {
    ...
    private static async addIpTransaction(ip: Partial<User.IIP>, trx: Transaction) {
        return DatabaseKnex.getInstance().insert(ip).into('USER_IP').transacting(trx);
    }

    private static async getIpTransaction(where: Partial<User.IIP>, trx: Transaction): Promise<User.IIP[]> {
        return DatabaseKnex.getInstance().select().from('USER_IP').where(where).transacting(trx);
    }

    private static async updateIpTransaction(ip: Partial<User.IIP>, trx: Transaction) {
        return DatabaseKnex.getInstance().update(ip).into('USER_IP').transacting(trx);
    }

    private static async addMacAddressTransaction(macAddress: Partial<User.IMacAddress>, trx: Transaction) {
        return DatabaseKnex.getInstance().insert(macAddress).into('USER_MACADDRESS').transacting(trx);
    }

    private static async getMacAddressTransaction(where: Partial<User.IMacAddress>, trx: Transaction): Promise<User.IMacAddress[]> {
        return DatabaseKnex.getInstance().select().from('USER_MACADDRESS').where(where).transacting(trx);
    }

    private static async updateMacAddressTransaction(macAddress: Partial<User.IMacAddress>, trx: Transaction) {
        return DatabaseKnex.getInstance().update(macAddress).into('USER_MACADDRESS').transacting(trx);
    }

    private static async addDeviceTransaction(device: Partial<User.IDevice>, trx: Transaction) {
        return DatabaseKnex.getInstance().insert(device).into('USER_DEVICE').transacting(trx);
    }

    private static async getDeviceTransaction(where: Partial<User.IDevice>, trx: Transaction): Promise<User.IDevice[]> {
        return DatabaseKnex.getInstance().select().from('USER_DEVICE').where(where).transacting(trx);
    }

    private static async updateDeviceTransaction(device: Partial<User.IDevice>, trx: Transaction) {
        return DatabaseKnex.getInstance().update(device).into('USER_DEVICE').transacting(trx);
    }

    private static async addOrUpdateIpTransaction(ip: Partial<User.IIP>, trx: Transaction) {
    ...
    private static async addMacAddressOrUpdateTransaction(macAddress: Partial<User.IMacAddress>, trx: Transaction) {
    ...
    private static async addDeviceOrUpdateTransaction(device: Partial<User.IDevice>, trx: Transaction) {
